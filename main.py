...
#ELEVENLABS_VOICE_ID = os.getenv("ELEVENLABS_VOICE_ID", "pqHfZKP75CvOlQylNhV4")
#ELEVENLABS_MODEL_ID = os.getenv("ELEVENLABS_MODEL_ID", "eleven_multilingual_v2")

# Configure Sarvam client (keeps same 'eleven_client' variable to avoid wider changes)
...
async def stream_elevenlabs_tts_old(text: str):
    """Stream PCM audio generated by ElevenLabs as SSE audio/pcm frames.

    This uses the ElevenLabs streaming conversion API to produce 16-bit PCM at 22.05 kHz.
    """
    if not sarvam_client or not ELEVENLABS_VOICE_ID:
        return

    try:
        audio_stream = sarvam_client.text_to_speech.convert(
        ...
        for chunk in audio_stream:
            ...
            print(f"[AGENT TO CLIENT via ElevenLabs]: audio/pcm: {len(chunk)} bytes.")
        ...
        async with sarvam_client.text_to_speech_streaming.connect(
            ...
            async for msg in ws:
                if isinstance(msg, AudioOutput):
                    # Sarvam gives base64; decode to bytes so the rest stays identical
                    try:
...
async def agent_to_client_sse(live_events, synthesize_audio: bool):
...
async def sse_endpoint(user_id: int, is_audio: str = "false"):
    ...
    async def event_generator():
        try:
            ...
            async for data in agent_to_client_sse(live_events, synthesize_audio=(is_audio == "true")):
                yield data
        except Exception as e:
